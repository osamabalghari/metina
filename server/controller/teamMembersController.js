...
exports.create = async function (req, res) {
	...
	utility.validate(data, ['user_id', 'team_id'])
	...
	let teamMembersData = await teamMembers.get({ team_id: data.team_id, user_id: data.user_id })
	if (teamMembersData.length) {
		// teamMembers is already exist
		utility.assert(!teamMembersData, `Team member already exist`)
	} else {

		teamMembersData = await teamMembers.create({ teamMember: data })
		console.log(`✅ Team member added: ${teamMembersData}`)
		return res.status(200).send({ data: teamMembersData })
	}
}
...
exports.get = async function (req, res) {
	...
	const teamData = await teamMembers.get({ id: id })
...
/*
 * get all team  members with team id
 */

exports.getAll = async function (req, res) {
	const id = req.params.id
	utility.assert(id, 'Please log in or specify a team ID')

	const teamData = await teamMembers.get({ team_id: id })
	for(let i=0; i< teamData.length; i++){
		teamData[i].user_id = await user.get({id:teamData[i].user_id})
		teamData[i].team_id = await team.get({id:teamData[i].team_id})
