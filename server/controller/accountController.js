...
exports.create = async function (req, res) {
	...
	await user.account.add({id: userData.id, account: accountData.id, permission: 'user'})
 ...
 */
exports.autoCreate = async function (req, res) {
	const data = req.body
	...
	if (!userData) {
		...
		// create the user and assign to account
		userData = !userData ? await user.create({user: data, account: accountData.id}) : userData
		await user.account.add({id: userData.id, account: accountData.id, permission: 'admin'})

		console.log(`✅ Account created for: ${data.email}`)

		// authenticate the user
		return await authController.adminSignup(req, res)
	}
}
...
exports.plan = async function (req, res) {
	...
	if (data.plan !== 'free') {
		if (data.stripe === undefined) {
			...
			stripeData.subscription = await stripe.customer.subscribe({id: stripeData.customer.id, plan: data.plan})
	...
	log.create({message: 'Customer added to plan', body: {plan: plan}, req: req})
...
exports.plan.update = async function (req, res) {
	...
	if (plan.id === 'free') {
		...
		if (accountData.stripe_subscription_id) {
			...
			await account.update({id: req.account, data: {stripe_subscription_id: null, plan: plan.id}})
		...
		if (accountData.stripe_subscription_id) {
			...
			if (subscription.status === 'trialing' || subscription.status === 'active') {
				subscription = await stripe.subscription.update({subscription: subscription, plan: plan.id})
				await account.update({id: accountData.id, data: {plan: plan.id}})
			} else if (subscription.status === 'canceled') {
				// user previously had a subscription, but is now cancelled - create a new one
				await account.update({id: req.account, data: {stripe_subscription_id: null, plan: 'free'}})
...
exports.upgrade = async function (req, res) {
	...
	if (data.stripe === undefined) {
		...
		// create a stripe customer and subscribe them to a plan
		stripeData.customer = await stripe.customer.create({email: accountData.email, token: data.token.id})
		stripeData.subscription = await stripe.customer.subscribe({id: stripeData.customer.id, plan: data.plan})
...
exports.card.update = async function (req, res) {
	...
	const customer = await stripe.customer.update({id: accountData.stripe_customer_id, token: req.body.token.id})
...
exports.users = async function (req, res) {
	return res.status(200).send({
		data: {
			...
			invites: await invite.get({account: req.account, returnArray: true})
		}
	})
}
...
exports.close = async function (req, res) {
	...
	if (accountData?.plan !== 'free' && accountData?.stripe_customer_id) await stripe.customer.delete(accountData?.stripe_customer_id)
