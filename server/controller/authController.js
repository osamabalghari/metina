...
exports.signin.otp = async function (req, res, next) {
	...
	try {
		...
		if (err.message === 'jwt expired') throw {message: 'Token has expired, please try signing in again'}
	}
	...
	if (req.body.code.length <= 6) {
		...
		// using backup code
		const verified = await user['2fa'].backup.verify({email: data.email, code: req.body.code})
		utility.assert(verified, 'Invalid verification code. Please try again')
	}
...
exports.get = async function (req, res) {
	...
	if (req.provider !== 'app') {
		...
		hasSocialToken = await token.verify({provider: req.provider, user: req.user})
	}
...
async function authenticate(req, res, userData, data) {
	...
	await token.save({provider: data?.provider || 'app', data: {access: jwt}, user: userData.id})
	user.update({id: userData.id, account: userData.account_id, data: {last_active: new Date(), disabled: false}})
