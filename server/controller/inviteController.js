...
exports.create = async function(req, res){
  ...
  const accountData = await user.get(req.account);
...
exports.addMember = async function(req, res){
  ...
  const accountData = await user.get({email: data?.email})
  utility.assert(accountData, 'Account does not exist');
  if (data.permission === 'owner')
    throw { message: 'Permission can be only owner or admin' }
  ...
  for (let email of emails){
    ...
    await mail.send({
      ...
      template: 'invite',
      content: {
        friend: accountData.name,
        id: data.id,
        email: data.email,
        domain: utility.validateNativeURL(req.body.url) || `${domain}/invite/user`
