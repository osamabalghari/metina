...
exports.get = async function (id) {
	const data = await db('account')
		.select('account.id', 'account.name', 'account.date_created', 'stripe_customer_id', 'stripe_subscription_id', 'plan', 'active', 'user.email as owner_email', 'user.name as owner_name')
...
exports.subscription = async function (id) {
	...
	if (accountData[0]?.plan !== 'free' && accountData[0]?.stripe_subscription_id) {
		...
		status = subscription?.status !== 'active' && subscription?.status !== 'trialing' ? subscription?.latest_invoice?.payment_intent?.status : subscription.status

		if (status !== 'active' && status !== 'trialing') await db('account').update({active: false}).where({id: id})
