...
exports.getUserFromToken = function (req) {
	...
	const decoded = jwt.decode(tokenBody)
	return decoded
}
	...
	return async function (req, res, next) {
		try {
			...
			if (type === 'Basic') {
				...
				if (process.env.ENABLE_API_LOGS === 'true') log.create({body: req.body, req: req, sendNotification: false, account: verified.account_id})
