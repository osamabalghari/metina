...
exports.save = async function ({provider, data, user}) {
	...
	const tokenData = await db('token').select('*').where({provider: provider, user_id: user})

	// update existing token
	if (tokenData.length) await db('token').update(data).where({id: tokenData[0].id, user_id: user})
	// create a new token
	else {
		data.id = uuidv4()
...
exports.verify = async function ({provider, user}) {
	const data = await db('token').select('id').where({provider: provider, user_id: user})
