...
exports.get = async function ({id, email, account, social, permission}) {
	const cols = ['id', 'name', 'email', 'date_created', 'support_enabled', 'last_active', 'onboarded', 'facebook_id', 'twitter_id', 'disabled', 'account_id', 'permission', 'default_account', 'password', '2fa_enabled']
...
exports.account.add = async function ({id, account, permission}) {
	return await db('account_users').insert({user_id: id, account_id: account, permission: permission})
}
...
exports.account.delete = async function ({id, account}) {
	return await db('account_users').del().where({user_id: id, account_id: account})
}
...
exports.password = async function ({id, account}) {
	const data = await db('user').select('password').join('account_users', 'account_users.user_id', 'user.id').where({id: id, 'account_users.account_id': account})
...
exports.password.verify = async function ({id, account, password}) {
	const data = await db('user').select('name', 'email', 'password').where({id: id, 'account_users.account_id': account}).join('account_users', 'account_users.user_id', 'user.id')
...
exports.update = async function ({id, account, data}) {
	...
	if (Object.keys(user).length) {
		await db('user')
			.update(user)
			.where('id', function () {
				this.select('user_id').from('account_users').where({account_id: account, user_id: id})
			})
	}
