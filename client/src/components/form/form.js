...
import { FormHeader, TextInput, NumberInput, EmailInput, URLInput, PhoneInput, DateInput, PasswordInput, HiddenInput, CardInput, Select, FormLink, Switch, FileInput, Fieldset, Button, ViewContext, useNavigate, ClassHelper, MultiSelect } from 'components/lib'

import { CardElement, useStripe, useElements } from '@stripe/react-stripe-js'
import Style from './form.tailwind.js'
import ButtonStyle from './file/file.tailwind.js';
...
function Form(props) {
	...
	useEffect(() => {
		...
		// otherwise, only init if no form set
			...
			// init credit card
			if (data?.token) {
				data?.plan?.default === 'free' ? setProcessCreditCard(false) : setProcessCreditCard(true)
			}
			...
			if (Object.keys(fileStore).length) {
				...
				for (let key in data) {
					...
					if (Array.isArray(data[key]) && data[key][0].hasOwnProperty('data')) {
						...
						// console.log('data[key]?.length', data[key]?.length, data[key], key)
						if(key === 'members'){
				...
				console.log('data[key]',formData, data)
	...
	// render the form
	return (
		<form action={props.action} method={props.method} onSubmit={submit} className={formStyle} encType={fileStore.length && 'multipart/form-data'} noValidate>
			...
			{fileStore?.logo?.length ? <div className='flex gap-[0.8rem] mb-3 flex-wrap'>
				{fileStore?.logo?.map(e => {
					return <><img className='h-24' src={e?.url} alt={e?.name} /> <Button
						icon='x'
						color='#d95565'
						width='auto !important'
						background={'bg-gray-300 rounded-[100%]'}
						size={17}
						className={ButtonStyle.fileListButton + ' ' + 'right-6 '}
						action={() => deleteFile(e)}
					/></>
				})} {props?.images &&<img className='h-24' src={process.env.REACT_APP_IMAGE_URL + props?.images} alt={''} />}	</div> : props?.images ?<img className='h-24 mb-3' src={process.env.REACT_APP_IMAGE_URL + props?.images} alt={''} /> :''}

			{props.buttonText && <Button color={props.destructive ? 'red' : 'green'} loading={loading} text={props.buttonText} action={submit} className={Style.button} fullWidth={!props.cancel} />}
