...
export function Fieldset(props) {
	...
	function change(index, state, option) {
		...
		if (props.type === 'radio') data = new Array(props.options?.length).fill(false)
	...
	return (
		<fieldset className={fieldsetStyle}>
			<Legend valid={valid} text={props.label} required={props.required} />

			<Grid cols="2">
				{props.options?.map((option, index) => {
					return <Input key={index} name={props.name} index={index} option={option} checked={checked[index]} callback={change} />
				})}
			</Grid>

			{!valid && <Error message={error} className={Style.errorMessage} />}
		</fieldset>
	)
}
