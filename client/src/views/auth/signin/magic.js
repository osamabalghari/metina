	...
	useEffect(() => {
		const verifyToken = async (token) => {
			try {
				...
				res.data['2fa_required'] ? navigate(`/signin/otp?token=${res.data.token}`) : res.status === 200 ? context.current.signin(res) : invalidLink()
			} catch (err) {
				invalidLink()
			}
		}
		...
		<Animate>
			<Row>
				<div style={{width: '28em', margin: '0 auto'}}>
					<Message type={message.type} title={message.title} text={message.text} buttonText={message.buttonText} buttonLink={message.buttonLink} />
				</div>
			</Row>
		</Animate>
	)
