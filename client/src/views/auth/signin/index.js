...
export function Signin(props) {
	...
	return (
		<Animate type="pop">
			<Row title="Sign in to Metrix" padding='py-0'>
				{magicLinkSent ? (
					<div style={{width: '28em', margin: '0 auto'}}>
						<Message title="Check Your Email" type="success" text="Please click the link in your email inbox to sign in." />
					</div>
				) : (
					<Card restrictWidth center>
						...
						<Form
							inputs={form}
							method="POST"
							url={useMagic ? '/api/auth/magic' : '/api/sigin'}
							buttonText={useMagic ? 'Send Magic Link' : 'Sign In'}
							callback={(res) => {
								useMagic ? setMagicLinkSent(true) : res.data['2fa_required'] ? navigate(`/signin/otp?token=${res.data.token}`) : navigate(authContext.signin(res))
							}}
						/>
